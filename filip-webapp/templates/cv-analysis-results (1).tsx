...
const CVAnalysisResults = () => {
  const [selectedCurrentSkills, setSelectedCurrentSkills] = useState([]);
  const [selectedRecommendedSkills, setSelectedRecommendedSkills] = useState([]);

  // Mock data - in real app this would come from props or API
  const uploadedFileName = "John_Doe_CV.pdf";
  const targetRole = "Senior Full Stack Developer specializing in React and Node.js, with expertise in cloud architecture and team leadership responsibilities";
  ...
  const toggleCurrentSkill = (skillName) => {
  ...
  const resetAnalysis = () => {
    // In real app, this would navigate back to upload form
    console.log('Resetting analysis - would navigate to upload form');
    alert('Would navigate back to upload form');
  };
    ...
    <div className="max-w-6xl mx-auto p-6 bg-gray-50 min-h-screen">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedCurrentSkills.map((skillName, index) => {
                    const skill = analysisResults.extractedSkills.find(s => s.name === skillName);
                    return (
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedRecommendedSkills.map((skillName, index) => {
                    const skill = analysisResults.skillsGap.find(s => s.skill === skillName);
                    return (
      ...
      <div className="mt-6 flex justify-center space-x-4">
        <button 
          ...
          disabled={selectedCurrentSkills.length === 0 && selectedRecommendedSkills.length === 0}
        >
          <Zap className="w-5 h-5" />
          <span>
            {selectedCurrentSkills.length > 0 || selectedRecommendedSkills.length > 0
              ? `Generate Learning Path (${selectedCurrentSkills.length + selectedRecommendedSkills.length} skills)`
              : 'Select Skills to Generate Learning Path'
            }
          </span>
        </button>
