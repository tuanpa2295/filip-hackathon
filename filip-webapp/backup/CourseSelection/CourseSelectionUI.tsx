...
import {
  ...
  ExternalLink, Zap, 
  BookOpen, DollarSign, BarChart3, Settings,
  ChevronDown, ChevronUp} from 'lucide-react';

interface CourseSelectionUIProps {
  selectedSkills: string[];
  onBack: () => void;
}
import { useNavigate } from 'react-router-dom';

const CourseSelectionUI: React.FC<CourseSelectionUIProps> = ({
  selectedSkills = [],
  onBack
}) => {
  // Use provided skills or default ones if empty
  ...
  const [expandedSkills, setExpandedSkills] = useState<Record<string, boolean>>({ 'Kubernetes': true });
  const [showTimelineAnalysis, setShowTimelineAnalysis] = useState<boolean>(false);
  const [analysisResults, setAnalysisResults] = useState<any>(null);

  // Mock course data for each skill
  const skillCourses: Record<string, Array<{
    id: string;
    title: string;
    provider: string;
    instructor: string;
    ...
    'JavaScript': [
      {
        ...
        level: 'Intermediate',
        format: 'Video + Projects',
        skills: ['ES6+', 'Async JS', 'DOM Manipulation', 'Fetch API'],
        description: 'Modern JavaScript from the beginning - no frameworks or libraries',
        highlights: ['5 real projects', 'ES6+ features', 'Error handling patterns'],
        prerequisites: ['Basic HTML/CSS'],
        outcomes: ['Build JS applications', 'Work with modern JS', 'Implement async patterns']
      }
    ],
    'React': [
      {
        ...
        level: 'Intermediate',
        format: 'Video + Projects',
        skills: ['React Hooks', 'Context API', 'Redux', 'React Router'],
        description: 'Dive in and learn React.js from scratch! Learn Hooks, Redux, React Router, and more!',
        highlights: ['Complete app', 'Custom hooks', 'Advanced patterns'],
        prerequisites: ['JavaScript ES6+'],
        outcomes: ['Build single-page applications', 'Manage complex state', 'Create reusable components']
      }
    ]
  };
  ...
  const toggleCourseSelection = (skillName: string, courseId: string) => {
  ...
  const analyzeTimeline = () => {
    ...
    const timeDiff = targetDate.getTime() - estimatedEndDate.getTime();
  ...
  const generateRecommendations = (riskLevel: string, daysDiff: number, totalHours: number, weeklyHours: number) => {
    ...
    if (riskLevel === 'high') {
      ...
      if (weeklyHours < 15) {
        const targetDateObj = new Date(learningPreferences.targetDate);
        const startDateObj = new Date(learningPreferences.startDate);
        const weeksDiff = Math.ceil((targetDateObj.getTime() - startDateObj.getTime()) / (7 * 24 * 60 * 60 * 1000));
        const suggestedHours = Math.ceil(totalHours / weeksDiff);

        recommendations.push({
          type: 'suggestion',
          title: 'Increase Weekly Hours',
          message: `Consider increasing to ${suggestedHours} hours/week`,
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold text-blue-600">{skills.length} Skills</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="border-t border-gray-200 p-6 pt-4">
                  <div className="grid gap-4">
                    {skillCourses[skill]?.map((course, courseIndex) => (
                      <div
                        key={courseIndex}
                        className={`border rounded-xl p-4 transition-all duration-200 cursor-pointer ${selectedCourses[skill]?.includes(course.id)
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-start space-x-4 flex-1">
                            <input
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                toggleCourseSelection(skill, course.id);
                              }}
                              readOnly
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-gray-900">${calculateTotalCost().toFixed(2)}</span>
              </div>
          ...
          {calculateTotalCost() > learningPreferences.budget && (
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
              ...
              <p className="text-orange-700 text-sm">
                Selected courses cost ${calculateTotalCost().toFixed(2)}, which exceeds your ${learningPreferences.budget} budget by ${(calculateTotalCost() - learningPreferences.budget).toFixed(2)}.
              </p>
            </div>
          )}
          ...
          <button
            ...
            style={{backgroundColor: '#2663ea', color: '#fff'}}
          >
          ...
          {/* Back Button */}
          <button
            className="w-full py-3 px-4 rounded-xl font-semibold transition-colors border border-gray-300 text-gray-700 hover:bg-gray-50 flex items-center justify-center space-x-2"
        ...
        <div className="mt-6 bg-white rounded-xl shadow-lg p-6 border-2 border-blue-200">
          ...
          <div className="space-y-4 mb-6 text-left">
            ...
            {analysisResults.recommendations.map((rec: any, index: number) => (
              <div key={index} className={`p-4 rounded-lg border ${rec.type === 'warning' ? 'border-red-200 bg-red-50' :
                  rec.type === 'caution' ? 'border-yellow-200 bg-yellow-50' :
                    rec.type === 'success' ? 'border-green-200 bg-green-50' :
                      'border-blue-200 bg-blue-50'
                }`}>
                <div className={`font-semibold mb-1 ${rec.type === 'warning' ? 'text-red-800' :
                ...
                <p className={`text-sm mb-2 ${rec.type === 'warning' ? 'text-red-700' :
                ...
                <div className={`text-xs font-medium ${rec.type === 'warning' ? 'text-red-600' :
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              ...
              style={{backgroundColor: '#1ea34a', color: '#fff'}}
              onClick={() => {
            ...
            <button
              ...
              onClick={() => setShowTimelineAnalysis(false)}
            >
              Adjust Selections
            </button>
